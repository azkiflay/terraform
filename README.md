...
As indicated by ("**resources": []**) in Figure 12, it can be seen that there are no resources according to the **terraform.state** file in the local host that belongs to the individual who issued the **terraform destroy**. Now, how can other team members know about the fact that all resources have been destroyed now? Surely, they would have known if they had access to the latest **terraform.state**. The problem arose because the **terraform.state** file exists on the local machine of the team member who destroyed the infrastructure. To solve this problem, the team can work using a centralized copy of **terraform.state**.
