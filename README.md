...
When "*terraform plan*" command is executed with a remote backend, Terraform downloads the *terraform.state* from the remote shared store, and uploads the state file to the remote store at the end of every "*terraform apply*" command. As stated earlier, only one Terraform process has the lock, so there will not be any race conditions among "*terraform apply*" commands issues by members of a team. Moreover, remote backends encrypt the "*terraform.state*" file in transit and at rest, keeping secrets of the Terraform resources secure. Effectively, the "*terraform.state*" file is kept always encrypted.
...
To that end, create the following AWS resources in **main.tf** within your current working directory. 
<!-- within a **backend** subdirectory in this example. The specifics of the file structure do not matter as long as your configuration files are in your current working directory.
-->
```bash
  terraform {
    required_providers {
...
```
You can see that the bucket created before (i.e., "*azkiflay-moodle-terraform-state*") is referenced in the "*terraform {}*" block. As a result, Terraform will not store its *terraform.state* on the local host any more. Instead, it will upload the *terraform.state* file to the S3 bucket after any changes take place locally, and download the latest copy of the state file from the S3 bucket before making any changes to infrastructure configuration. The result is a consistent and secure infrastructure state that is managed collaboratively by members of a team. No more "*terraform apply*" conflicts or state file inconsistency.
