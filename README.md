...
As indicated by ("**resources": []**) in Figure 12, it can be seen that there are no resources according to the **terraform.state** file in the local host that belongs to the individual who issued the **terraform destroy**. Now, how can other team members know about the fact that all resources have been destroyed now? Furthermore, there would be conflicting and corrupted infrastructure state if another member of the team issued *terraform apply* to create new resources, not knowing another colleague had issued "*terraform destroy*". In general, how can members of the team know the correct and latest state of the infrastructure?

Surely, they would have known if they had access to the latest **terraform.state**. The problem arose because the **terraform.state** file exists on the local machines of the team members who made changes to the infrastructure. To solve this coordination problem, they can work using a centralized copy of **terraform.state**.
